...
class Server_Process(object):
    def __init__(self, env, BufferSize, Server_Idle_Periods, Packet_Delay):
        self.env = env
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                while self.len > 0:
                    # print("In SERVICE: ", "no. of pkts in system: ", self.len, len(self.queue))
                    packet = self.queue.pop()
                    yield self.env.timeout(random.expovariate(G.MU)) # server processing packets
...
class Arrival_Process(object):
    ...
    def run(self):
        ...
        while True:
            ...
            self.server_process.TotalPacketCount +=1

            #packet will drop for the following condition
            if(self.server_process.len > self.server_process.BufferSize):
              #packet is being dropped, so drop count is increasing
              # print("dropping", "Drop Count: ",self.server_process.DropPacketCount, "Packets in the system: ", self.server_process.len)
              self.server_process.DropPacketCount += 1
            ...
            #arrival process is generating packets. Hence, no. of packets in the system is increasing
            #no. of packets in the system is denoted by len
...
def main():
    ...
    print ("{0:<9} {1:<9} {2:<9} {3:<9}".format(
        "Lambda", "BufferSize", "BlockProbability(Sim)", "BlockProbability(Theory)"))
    ...
    for arrival_rate in [0.1, 0.4, 0.8, 0.9]:
      for buffer_size in [1,10,50]:
          ...
          server_process = Server_Process(env, buffer_size,Server_Idle_Periods, Packet_Delay)
          arrival_process = Arrival_Process(env, arrival_rate, server_process, Server_Idle_Periods, Packet_Delay)
          env.run(until=G.SIM_TIME)
          # print(server_process.DropPacketCount)
          #theoritical result
          rho = arrival_rate/G.MU

          theo = ((1 - rho)*(rho ** buffer_size))
          theo = theo / (1 - (rho ** (buffer_size+1)))

          print ("{0:<9.3f} {1:<12} {2:<25.3f} {3:<25.3f}".format(
              round(arrival_rate, 3),
              int(buffer_size),
              round(server_process.DropPacketCount/server_process.TotalPacketCount, 3),
              round(theo, 3)))
